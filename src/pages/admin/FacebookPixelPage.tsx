      ...
      <div className="bg-surface dark:bg-card rounded-sm border border-border/30 shadow-sm p-4 md:p-5">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-[11px] text-muted-foreground mt-0.5">সার্ভার-সাইড ইভেন্ট ট্র্যাকিং কনফিগার করুন</p>
          </div>
          <button
            onClick={handleSave}
            disabled={saving}
      ...
      <div className="bg-surface dark:bg-card rounded-sm border border-border/30 shadow-sm p-5 md:p-6 space-y-4">
        ...
        <div className="space-y-3">
          <div>
            ...
            <input type="text" value={pixelId} onChange={e => setPixelId(e.target.value)} placeholder="যেমন: 123456789012345"
          ...
          <div>
            ...
            <input type="password" value={accessToken} onChange={e => setAccessToken(e.target.value)} placeholder="আপনার Access Token দিন..."
              className="w-full bg-muted/30 border border-border/40 rounded-sm px-3 py-2.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-gold/20 transition-all font-mono" />
          </div>
        </div>
      </div>
            ...
            >
              <div className="flex items-center gap-2.5">
                <div className={`w-8 h-8 rounded-sm flex items-center justify-center ${
                  enabledEvents.includes(ev.id) ? 'bg-accent/10' : 'bg-muted/50'
                }`}>
                  <ev.icon className={`h-4 w-4 ${enabledEvents.includes(ev.id) ? 'text-gold' : 'text-muted-foreground'}`} />
                </div>
                <div>
