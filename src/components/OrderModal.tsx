        ...
        >
          ...
          <div className="p-5 pt-0 space-y-5">
            ...
            <div className="space-y-3">
              ...
              <div>
                <input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="মোবাইল নম্বর *" className={`w-full bg-transparent border rounded-sm px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-gold/20 focus:border-gold/40 transition-all ${touched && errors.phone ? 'border-destructive/60 bg-destructive/5' : 'border-border/60'}`} maxLength={15} />
                <ErrorMessage error={errors.phone} />
              </div>
              <div>
                <textarea value={address} onChange={(e) => setAddress(e.target.value)} placeholder="সম্পূর্ণ ঠিকানা *" rows={2} className={`w-full bg-transparent border rounded-sm px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-gold/20 focus:border-gold/40 transition-all resize-none ${touched && errors.address ? 'border-destructive/60 bg-destructive/5' : 'border-border/60'}`} maxLength={500} />
            ...
            {/* Payment Tabs */}
            <div>
              <div className="flex rounded-sm bg-muted/50 p-1 gap-1 border border-border/30">
                <button onClick={() => setTab('cod')} className={`flex-1 py-2.5 rounded-sm text-sm font-medium transition-all ${tab === 'cod' ? 'bg-surface shadow-sm text-foreground border border-border/30' : 'text-muted-foreground hover:text-foreground'}`}>
                  ক্যাশ অন ডেলিভারি
                </button>
